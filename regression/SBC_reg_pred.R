...
finalMatrix  <- foreach(i = 1:reps, .packages = c("rstan", "tcltk"), 
                        ...
                        .verbose = TRUE) %dopar% {
                          ...
                          warm <- warm.init
                          counter <- 1
                          fit <- sampling(simple_reg_model_pred, 
                                          data = list(X = X[, , i], n_ind = n_ind, y = ysim[, i], 
                                                      N_test = n_test, 
                                                      X_test = X_test[, , i]),
                         ...
                         ## note 2 * L, was L but still signs of autocorrelation.
                          ...
                          fit_b0 <- rstan::extract(fit, pars = "b0")[[1]]
                          fit_b1 <- rstan::extract(fit, pars = "b1")[[1]]
                          fit_b2 <- rstan::extract(fit, pars = "b2")[[1]]
                          fit_b3 <- rstan::extract(fit, pars = "b3")[[1]]
                          fit_b4 <- rstan::extract(fit, pars = "b4")[[1]]
                          fit_b5 <- rstan::extract(fit, pars = "b5")[[1]]
                          fit_sigma <- rstan::extract(fit, pars = "sigma")[[1]]
                          fit_yrep <- rstan::extract(fit, pars = "y_test")[[1]]
                          ...
                          fit_yrep <- fit_yrep[(1:L) * thin, ]
                          
                          ## bias
                          bias_b0 <- summary(fit, pars = "b0")$summary[1] - b0[i]
                          bias_b1 <- summary(fit, pars = "b1")$summary[1] - b1[i]
                          bias_b2 <- summary(fit, pars = "b2")$summary[1] - b2[i]
                          bias_b3 <- summary(fit, pars = "b3")$summary[1] - b3[i]
                          bias_b4 <- summary(fit, pars = "b4")$summary[1] - b4[i]
                          bias_b5 <- summary(fit, pars = "b5")$summary[1] - b5[i]
                          bias_sigma <- summary(fit, pars = "sigma")$summary[1] - sigma[i]
                          
                          
                          tempMatrix <- matrix(NA, ncol = 15 + n_test, nrow = 1)
                          ...
                          tempMatrix[1, 16:(15 + n_test)] <- colMeans(fit_yrep) - ysim_test[, i]
