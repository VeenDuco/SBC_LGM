...
model {
  ...
  for (n in 1:n_ind) {
    ...
    // priors
    target += gamma_lpdf(sigma[1] | 1, 1);
    target += gamma_lpdf(sigma[2] | 1, 1);
    target += lkj_corr_cholesky_lpdf(Lcorr | 1);
    
    
    // target += normal_lpdf(tau_i | 0, psi11);
    target += normal_lpdf(alpha1 | 0, 100000);
    target += normal_lpdf(alpha2 | 0, 100000);
    target += gamma_lpdf(theta | 1, 1);
    ...
    for(i in 1:n_ind){
    for(j in 1:n_time) {
    target += normal_lpdf(y[i, j] | 1 * Intercept[i] + x[j] * Slope[i], theta); 
    }
    }
